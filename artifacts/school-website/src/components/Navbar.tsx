...
export function Navbar({ banner, onDismissBanner }: NavbarProps) {
  ...
  return (
    <nav
      ...
      data-testid="navbar"
    >
      {/* Announcement Banner — inside fixed nav so it scrolls with nav */}
      {banner && (
        <div className="bg-[hsl(49,87%,60%)] text-[hsl(209,64%,22%)] text-xs sm:text-sm font-medium px-4 py-2 flex items-center justify-center gap-3">
          <span className="text-center leading-snug">{banner}</span>
          <button
      ...
      {/* Main nav bar */}
      <div
        className={`bg-[hsl(209,64%,28%)] transition-all duration-300 ${
          scrolled ? "shadow-lg py-2" : "py-3 sm:py-4"
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              className="xl:hidden text-white p-1.5 hover:bg-white/10 rounded transition-colors"
      ...
      {/* Mobile Drawer */}
      {mobileOpen && (
        <div className="xl:hidden bg-[hsl(209,64%,22%)] border-t border-white/10 max-h-[85vh] overflow-y-auto">
          <div className="px-4 py-4 space-y-1">
                ...
                {item.dropdown ? (
                  <>
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-3 text-sm text-white/90 hover:bg-white/10 rounded transition-colors"
                    ...
                    {activeDropdown === item.label && (
                      <div className="ml-4 mt-1 space-y-1">
                        {item.dropdown.map((sub) => (
                          <Link key={sub.href} href={sub.href} className="block px-3 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/10 rounded transition-colors" data-testid={`mobile-nav-sub-${sub.href}`}>
                            {sub.label}
                          </Link>
                        ))}
                      </div>
                  ...
                  <Link href={item.href!} className="block px-3 py-3 text-sm text-white/90 hover:bg-white/10 rounded transition-colors" data-testid={`mobile-nav-${item.label}`}>
                    {item.label}
                  </Link>
                )}
              </div>
            ...
            <div className="pt-3 mt-3 border-t border-white/10">
              <Link href="/admissions" className="block w-full text-center px-4 py-3 bg-[hsl(49,87%,60%)] text-[hsl(209,64%,22%)] font-semibold rounded text-sm hover:opacity-90 transition-opacity">
